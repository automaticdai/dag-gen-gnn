# show_fixed.py —— Visualize all edges and highlight cross-layer (skip-layer) edges with "arcs"
...
PATHS = [
    r"E:\PythonProject7\src\generated5\gen_like_1756894512.gpickle",
    r"E:\DAG\data\gpickle1\Tau_4_Tau_0.gpickle",
]
SAVE_PNG = False   # True saves the PNG to the same directory with the suffix _vis.png
# ====================================
...
def print_time_stats(G: nx.DiGraph, name: str = ""):
    ...
    if ts:
        ...
        print(f"[{name}] no（critical_time/time/weight/C/t）。")

if __name__ == "__main__":
    if not PATHS:
        print("PATHS is empty"); raise SystemExit(1)
    for p in PATHS:
        G = load_graph(p)
