# infer.py —— Generate a similar DAG using the reference graph; time is quantized into integers per "node" and the total duration is calibrated (Scheme B)
...
def s_from_graph(G: nx.DiGraph) -> Tuple[torch.Tensor, List[int], int, float]:
    ...
    if not nx.is_directed_acyclic_graph(G):
        raise ValueError("reference graph is not DAG")
...
def _enforce_single_source_sink(G: nx.DiGraph) -> tuple[nx.DiGraph, int]:
    ...
    # 1) And sources: For each additional source s, try to pick a u→s from the previous layer (ΔL=1)
    ...
    # 2) Merge sinks: For each additional sink t, try to connect it to the next layer v (t→v, ΔL=1)
...
def print_time_report(G: nx.DiGraph, ref_total_T: float | None=None, eps: float=1e-6):
    ...
    if not ts:
        print("[time] no critical_time"); return
...
if __name__ == "__main__":
    # reference
    REF_PATH = Path(r"E:\DAG\data\gpickle1\Tau_4_Tau_0.gpickle")
